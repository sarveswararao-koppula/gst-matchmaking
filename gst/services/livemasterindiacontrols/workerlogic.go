...
// SubcriberHandler ...
func SubcriberHandler2(data string) (string, error) {
	...
	var fyLatestFiling string
	if wr.APIName == "challan" {
		fyLatestFiling, err = Challan2(wr)
		// fmt.Println("fyLatestFiling subscriberhandler=",fyLatestFiling)
		return fyLatestFiling, err
	}
	return fyLatestFiling, nil
}

// Challan2 ...
func Challan2(work WorkRequest) (string, error) {
	...
	//changesrelatedchallanfinancialyear
	fy0errorflag := 0
	...
	for _, fy := range fys {
		...
		if (fy0errorflag == 0 && fy.fy == "fy1" && (work.Modid == "merpcsd" || work.Modid == "merpnsd" || work.Modid == "weberp")) || fy.fy == "fy2" || fy.fy == "fy3" {
			s3log.APIHit = "N"
			Write2S3(&s3log)
			Write2Kibana(&s3log)
			continue
	...
	// if fyLatestFiling != "" {
	// 	date, err := time.Parse("2006-01-02", fyLatestFiling)
	// 	if err != nil {
	// 		fmt.Println("Error parsing date:", err)
	// 		return "", err
	// 	}
	// 	fyLatestFiling = date.Format("02-01-2006")
	// }

	if fyLatestFiling != "" {
		parsedDate, err := time.Parse("2006-01-02", fyLatestFiling)
		if err == nil {
